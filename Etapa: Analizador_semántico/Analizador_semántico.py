...
class Seg_Parametros(Nodo):
    ...
    def __repr__(self):
        objeto = ('Parametros' '\n' + ' Tipo: ' + str(self.data) + ' Id: ' + str(self.id) + ' Funcion: ' + str(
            self.tipo))
        return objeto
        ...
        while contador < self.partederecha:
            for obj in self.variables:
                ...
                for obj2 in list_ref:
                    if obj.cad == obj2.var and obj.contexto == obj2.contexto and obj.contexto == globals()[
                        'cont'] or obj.cad.isdigit() and banderadigito == 0:
                        if vuelta == 0:
                            if bandera == 0:
                                if obj.cad.isdigit():
                                    cad = cad + '' + obj.cad
...
def rev_regl():
    file = open('C:\\Users\\andre\\OneDrive\\Escritorio\\Seminario de Traductores de Lenguaje 2\\Analizador semántico\\compilador.lr', 'r')
    line = file.readlines()
    for m in line:
        m = m.rstrip()
...
def auxil_regl():
    n = 1
    file = open('C:\\Users\\andre\\OneDrive\\Escritorio\\Seminario de Traductores de Lenguaje 2\\Analizador semántico\\rgl.txt', 'r')
    line = file.readlines()
    for m in line:
        m = m.rstrip()
        ...
        try:
            if lista_pila[14].cad == 'return':
                ...
                if lista_pila[2] == 'Definicion':
                    ...
                    globals()['termino'] = Node(Termino(self.data, self.tipo, 'Retorno ' + lista_pila[4].cad),
                                                parent=raiz)
            else:
            ...
            for obj in lista_variables:
                if self.data.cad == obj.tipo.cad and obj.lv == globals()['cont']:
                    ...
                    if cont == 0:
                        cont = 1
                        list_er.append(
                            'Error de asignacion ' + str(self.data.cad) + ' variable inexistente o fuera de contexto')
                        aux = 'mmm'
                        aux2 = 'mmm'
                        anadidos += 1
                    else:
                        ...
                        try:
                            ...
                            if flag == 1:
                                if (self.cadena_analizada[self.i]) == '(':
                                    ...
                                    print('No hay')
                                    list_er_lex.append(
                                        'Punto y coma faltantes: ' + str(list_lex[-2].cad) + ' ' + self.tmp)
                ...
                if list_lex[-1].tipo == 'Identificador':
                    if entrada[actual + 1] == '=':
                        if ';' in entrada[actual + 2]:
                            ...
                            aumento += 1
                            if entrada[actual + aumento] == '+' or entrada[actual + aumento] == '-' or entrada[
                                actual + aumento] == '*' or entrada[actual + aumento] == '/':
                                while aux == 0:
                                    if entrada[actual + aumento] == '+' or entrada[actual + aumento] == '-' or entrada[
                                        actual + aumento] == '*' or entrada[actual + aumento] == '/':
                                        aumento += 1
                                        ...
                                        if list_lex[-1].pos == 0 or list_lex[-1].pos == 1 or list_lex[-1].pos == 2:
                                            ...
                                            aux = 1
                                            list_lex.pop()
                                            list_er_lex.append('Punto y coma faltantes: ' + str(entrada[actual]) + str(
                                                entrada[actual + 1]) + str(entrada[actual + 2]))
                                            break
                                    else:
                                        aux = 1
                                        list_lex.pop()
